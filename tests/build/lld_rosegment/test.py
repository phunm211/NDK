...
def run_test(
    ...
    if config.abi == "riscv64":
        verifier = FlagVerifier(test_dir / "project", ndk_path, config).with_api(35)
        verifier.expect_not_flag("-Wl,--no-rosegment")
        return verifier.verify().make_test_result_tuple()

    verifier = FlagVerifier(test_dir / "project", ndk_path, config).with_api(29)
    ...
    verifier = FlagVerifier(test_dir / "project", ndk_path, config).with_api(30)
    verifier.expect_not_flag("-Wl,--no-rosegment")
    return verifier.verify().make_test_result_tuple()
