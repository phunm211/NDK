...
async def fixture_android_ci_client(
    aiohttp_client: type[TestClient[Request, Application]],
...
async def test_fetch_artifact(
    android_ci_client: TestClient[Request, Application],
) -> None:
...
async def test_fetch_artifact_chunked(
    android_ci_client: TestClient[Request, Application],
) -> None:
...
async def test_downloader_progress_reports(
    android_ci_client: TestClient[Request, Application],
) -> None:
    """Tests that progress is reported when using ArtifactDownloader."""
    downloader = FakeDownloader(TEST_TARGET, TEST_BUILD_ID, TEST_ARTIFACT_NAME)
...
async def test_real_artifact() -> None:
    ...
    async with ClientSession() as session:
        contents = await fetch_artifact("linux", "13288689", "logs/SUCCEEDED", session)
        assert contents == b"1743205645\n"
