...
class ArgumentParser(argparse.ArgumentParser):
    ...
    def __init__(self):
        ...
        self.add_argument("--adb", dest="adb_path", help="use specific adb command")
    ...
    def parse_args(self, args=None, namespace=None):
        ...
        try:
            subprocess.check_output([adb_path, "version"], stderr=subprocess.STDOUT)
...
def start_gdbserver(
    ...
    forward_gdbserver_port(
        device, local=port, remote="localfilesystem:{}".format(chroot + debug_socket)
    )
    ...
    if lldb:
        ...
        gdbserver_output_path = os.path.join(tempfile.gettempdir(), "gdbclient.log")
    ...
    return device.shell_popen(
        gdbserver_cmd, stdout=gdbserver_output, stderr=gdbserver_output
    )
...
def find_file(device, executable_path, sysroot, run_as_cmd=None):
    ...
    def generate_files():
        ...
        try:
            ...
            raise RuntimeError(
                "Failed to copy '{}' to temporary folder on "
...
def find_executable_path(device, executable_name, run_as_cmd=None):
    ...
    try:
        ...
        raise RuntimeError(
            "Could not find executable '{}' on " "device".format(executable_name)
        )
